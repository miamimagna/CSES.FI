...
signed main()
{
    ...
    while (q--)
    {
        ...
        if (type == 2)
        {
            ...
            cin >> a >> b;
            int lca = lc.lca(a, b);
            // get the max from a to lca
            int mxa = -1e9;
            int rootlca = heavy.find(lca);
            while (a != lca)
            {
            ...
            while (b != lca)
            {
                ...
                int sz = heavy.getsize(root);
                if (sz == 1)
                    mxb = max(mxb, values[b]), b = parent[b];
                else if (rootlca == root)
                    mxb = max(mxb, heavyseg[root].sum(-depth[top_dsu[root]] + depth[lca], -depth[top_dsu[root]] + depth[b])), b = lca;
                else
                    mxb = max(mxb, heavyseg[root].sum(0, -depth[top_dsu[root]] + depth[b])), b = parent[top_dsu[root]];
            }
            cout << max(mxa, mxb) << ' ';
        }
        // replace node value
