...
void dfs1(int i, int p, v<v<int>> &adj, v<int> &parent)
{
    parent[i] = p;
    for (int child : adj[i])
        if (child != p)
            dfs1(child, i, adj, parent);
}
int dfs2(int i, int p, v<v<int>> &adj, v<int> &value, v<int> &prefix)
{
    for (int child : adj[i])
        if (child != p)
            value[i] += dfs2(child, i, adj, value, prefix);
    prefix[i - 1] = value[i];
    return value[i];
}
signed main()
{
    ...
    while (q--)
    {
        ...
        int lca = lc.lca(a, b);
        value[a]++, value[b]++;
        value[lca]--, value[parent[lca]]--;
    }
    v<int> prefix(n);
    dfs2(1, 0, adj, value, prefix);
    for (auto x : prefix)
        cout << x << ' ';
    cout << endl;
}
